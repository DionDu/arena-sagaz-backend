...
- [X] T001 Ler `gerador_dados/jogo_pontinhos/minimax_pontinhos.py` na íntegra para mapear assinatura atual de `minimax()` e `avaliar()` antes das alterações requeridas por D3 (DI obrigatória)
- [X] T002 Ler `gerador_dados/jogo_pontinhos/contrato_codificacao_pontinhos.json` na íntegra para mapear encoding do contexto 3 "Partidas / uso interativo" — domínio `{-1, 0, 1, 8}`, normalização `8→0, -1→1, 9→1` — obrigatório por CLAUDE.md antes de qualquer pipeline de inferência ou construção de estado de tabuleiro
- [X] T003 [P] Verificar presença de `modelos/pontinhos_pequeno_profundidade_6.tflite`, `modelos/pontinhos_pequeno_profundidade_7.tflite` e `modelos/pontinhos_pequeno_profundidade_9.tflite` antes de implementar carregamento TFLite
...
- [X] T006 Criar `gerador_dados/jogo_pontinhos/tipos_pontinhos_3_4.py` com enums `NivelDificuldade(str, Enum)`, `CodigoSituacao(str, Enum)`, `CodigoAcao(str, Enum)` e constante `MAPEAMENTO_NIVEIS` (data-model.md § Enumerações e MAPEAMENTO_NIVEIS)
- [X] T007 Adicionar `@dataclass ConfiguracaoAgente` com campos `nivel_dificuldade`, `caminho_modelo_cnn`, `profundidade_minimax`, `percentual_aleatoriedade`, `seed_aleatoriedade`, `verbose` e `__post_init__` derivando defaults de `MAPEAMENTO_NIVEIS` com validações ValueError em `gerador_dados/jogo_pontinhos/tipos_pontinhos_3_4.py`
- [X] T008 Adicionar `@dataclass(frozen=True) MetadadosTurno` com campos `id_partida`, `id_jogada`, `id_jogador`, `nu_jogador`, `ts_jogada`, `nu_timer_ms: int | None = None` e `__post_init__` com ValueError em `nu_jogador ∉ {1,-1}` e `nu_timer_ms < 0` em `gerador_dados/jogo_pontinhos/tipos_pontinhos_3_4.py`
- [X] T009 Adicionar `@dataclass ResultadoJogada` com todos os campos comuns (`id_partida`, `id_jogada`, `id_jogador`, `nu_jogador`, `co_situacao`, `co_acao`, `co_aresta`, `ar_tabuleiro_antes`, `ar_tabuleiro_apos`, `nu_placar_jogador_antes`, `nu_placar_jogador_apos`, `ts_jogada`, `nu_timer_ms`, `nu_tempo_calculo_ms`) e campos opcionais (`nu_profundidade_minimax`, `ar_score_minimax`, `ar_probabilidade_cnn`, `js_extra`) em `gerador_dados/jogo_pontinhos/tipos_pontinhos_3_4.py`
- [X] T010 Adicionar `@dataclass(frozen=True) Estrutura` com campos `tipo: Literal["corrente", "ciclo", "ramificada", "isolada"]`, `caixas: tuple[tuple[int, int], ...]`, `extremidades: tuple[tuple[int, int], ...]` e propriedades `tamanho` e `eh_corrente_longa` em `gerador_dados/jogo_pontinhos/tipos_pontinhos_3_4.py`
- [X] T011 Adicionar helpers públicos `array_31_com_nan() -> np.ndarray` e `contar_caixas_jogador(estado, jogador) -> int` em `gerador_dados/jogo_pontinhos/tipos_pontinhos_3_4.py`
- [X] T012 Adicionar type alias `FuncaoAvaliacao = Callable[[EstadoTabuleiro, int, int], int]` e parâmetro `fn_avaliacao: FuncaoAvaliacao = avaliar` à função `minimax()` em `gerador_dados/jogo_pontinhos/minimax_pontinhos.py` mantendo default compatível com chamadores legados (D3)
...
- [X] T044 Criar `docs/jogo_pontinhos/documentacao_ia_pontinhos_3_4.md` com: visão geral do agente híbrido; arquitetura dos 4 passos; modos de operação e adversários; 4 níveis de dificuldade com mapeamento canônico; timer cooperativo (D10) e hierarquia P1/P2/P3; exemplos de uso (sem timer, timer largo, timer apertado, timer extremo); design do módulo `correntes_pontinhos_3_4.py` e seu uso futuro previsto em `avaliador_partidas_pontinhos.py` / `visualizador_pontinhos.py`; instruções de execução dos testes (FR-031)
- [X] T045 Atualizar `docs/historico_decisoes.md` registrando entrada para timer cooperativo D10: data 2026-05-02, contexto (necessidade de degradação graciosa para app Flutter), decisão (checkpoints síncronos com `time.monotonic_ns()`), alternativas rejeitadas (`signal.SIGALRM` POSIX-only, `threading.Timer` sem cancelamento real, `concurrent.futures` sem interrupção, async com custo de refatoração), motivo (portabilidade Windows/macOS/Linux, determinismo, custo zero no caminho feliz) (FR-030)
- [X] T046 [P] Executar `pytest tests/unitarios/jogo_pontinhos/ --cov=gerador_dados/jogo_pontinhos/ --cov-report=term-missing` e verificar cobertura ≥ 90% em `ia_pontinhos_3_4.py`, `correntes_pontinhos_3_4.py`, `cnn_inferencia_pontinhos_3_4.py` e `tipos_pontinhos_3_4.py` (SC-008) — **resultado: cnn 100%, correntes 91%, ia 92%, tipos 100% (média 94%)**
- [⚠] T047 [P] Executar `tests/unitarios/jogo_pontinhos/test_contrato_codificacao_pontinhos.py` — **FALHA PRE-EXISTENTE** detectada em `test_hash_backend_igual_ao_frontend`: backend (`7399222b…`) e frontend (`8fee0543…`) divergem. Drift anterior à feature 003; deve ser resolvido em sessão dedicada copiando o JSON do backend para `arena-sagaz-frontend/assets/jogos/pontinhos/contrato_codificacao_pontinhos.json` (CLAUDE.md obriga sincronia)
- [X] T048 Validar manualmente exemplos 1–4 de `specs/003-jogador-hibrido/quickstart.md` — smoke test em tabuleiro vazio (sem timer) executou Passo 3+4 com `co_acao=CNN_E_MINIMAX`; testes unitários validam P1/P2/P3 com mock de timer (`test_timer_p3_aleatoria_quando_estoura_imediatamente`, `test_timer_estoura_apos_cnn_retorna_p2`, `test_timer_largo_retorna_p1`)
